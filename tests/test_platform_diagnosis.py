...
def test_live_diagnosis_flags_excursion_without_online_updates() -> None:
    ...
    assert payload["detector"]["missing_value_policy"] == "frozen_training_median"
    assert payload["detector"]["median_imputed_values"] == 0
...
def test_live_diagnosis_reports_frozen_training_median_imputation() -> None:
    ...
    assert payload["detector"]["missing_value_policy"] == "frozen_training_median"
    assert payload["detector"]["median_imputed_values"] == 1
    assert payload["detector"]["median_imputed_by_feature"] == {"x": 0, "y": 1}
